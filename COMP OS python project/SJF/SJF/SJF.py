...
Pr1 = sjf() #setting each process accordingly to its data, their own wait time, turnaround time, response time, etc
Pr2 = sjf()
Pr3 = sjf()
Pr4 = sjf()
Pr5 = sjf()
...
while(ready or running or io):
    if(not running): #checks if running is empty
        if(ready): #if ready has any values
            ready.sort() #sorts ready so smallest is first
            running.append(ready.pop(b)) #adds the first value of ready to running
            if(running): #if running has values prints all the values in it
                Response(Ttr) #gets response time
                print("Running List:")
                print(running[0])
            else:
                print("Running burst is empty")
            if(ready): #prints all values in ready
                c = 0
                print("Ready List:")
                while c < len(ready):
                    print(ready[c])
            ...
            if(io): #prints all the values in io
                c = 0
                print("IO burst:")
                while c < len(io):
                    print(io[c])
        ...
        if(running[0][0] > 0): #if the current running burst is greater than 0 decrements it and increases turnaround time
            running[0][0]-=1
            Ttr +=1
   
        else:
            
            new_word = running[0] #removes burst time 
            new_word.pop(0)
            io.append(running.pop())
        if(ready and not running):
            ready.sort() #sorts ready
            running.append(ready.pop(b)) #moves to running
            if(running): 
                Response(Ttr)
                print("Running List:")
                print(running[0])
            ...
            Ttr +=1
    if(io):
        IoCount(io,ready, Ttr) #decrenents all values in io
        counter(Ttr)
waiting()
average()
print(terminate)
